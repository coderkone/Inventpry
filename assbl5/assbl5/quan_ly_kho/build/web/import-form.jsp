...
<body>
        ...
        <div class="card card-main">
            <div class="card-body p-4">
                <div class="d-flex justify-content-between align-items-center mb-4">
                    <div>
                        <h3 class="fw-bold mb-1">Tạo phiếu nhập</h3>
                        <p class="text-muted mb-0">Sau khi lưu, hệ thống tự cộng tồn kho và ghi vào lịch sử kho.</p>
                    </div>
                </div>
                <c:if test="${param.msg == 'created'}"><div class="alert alert-success">Tạo phiếu nhập thành công. Mã phiếu: <b>${param.code}</b></div></c:if>
                <c:if test="${not empty error}"><div class="alert alert-danger">${error}</div></c:if>

                <form action="imports" method="post" id="receiptForm">
                        ...
                        <div class="col-md-6">
                            ...
                            <select class="form-select" name="supplierId" required>
                                ...
                                <c:forEach var="s" items="${suppliers}">
                                    <option value="${s.maNhaCungCap}">${s.tenNhaCungCap}</option>
                                </c:forEach>
                            </select>
                        </div>
                        <div class="col-md-6">
                            <label class="form-label">Ghi chú</label>
                            <input class="form-control" name="note" placeholder="Ghi chú phiếu nhập">
                        </div>
                    </div>
            ...
            <td>
                <select class="form-select product-select" name="productId" onchange="syncPrice(this)">
                    ...
                    <c:forEach var="p" items="${products}">
                        <option value="${p.maSanPham}" data-import-price="${p.giaNhap}" data-stock="${p.soLuongTon}">${p.maHang} - ${p.tenSanPham}</option>
                    </c:forEach>
                </select>
            </td>
    ...
    <script>
        ...
        function syncPrice(select) {
            ...
            row.querySelector('.price-input').value = opt.dataset.importPrice || 0;
