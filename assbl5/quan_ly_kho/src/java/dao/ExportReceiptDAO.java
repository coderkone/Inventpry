...
public class ExportReceiptDAO {
    ...
    public String createExportReceipt(long userId, String customerName, String note, List<ReceiptItem> items) throws Exception {
        Connection con = null;
        try {
            con = DBConnection.getConnection();
            con.setAutoCommit(false);

            long receiptId = getNextId(con, "phieu_xuat", "ma_phieu_xuat");
            long detailId = getNextId(con, "chi_tiet_phieu_xuat", "ma_chi_tiet_phieu_xuat");
            long historyId = getNextId(con, "lich_su_kho", "ma_lich_su");
            String receiptCode = buildCode("PX", receiptId);
            double total = 0;
            for (ReceiptItem item : items) {
                total += item.getTotalPrice();
            }

            String insertReceipt = "INSERT INTO phieu_xuat (ma_phieu_xuat, ma_phieu, ma_nguoi_tao, ngay_xuat, ten_khach_hang, tong_tien, ghi_chu, trang_thai) VALUES (?, ?, ?, GETDATE(), ?, ?, ?, 'HOAN_THANH')";
            ...
            String insertDetail = "INSERT INTO chi_tiet_phieu_xuat (ma_chi_tiet_phieu_xuat, ma_phieu_xuat, ma_san_pham, so_luong, don_gia, thanh_tien) VALUES (?, ?, ?, ?, ?, ?)";
            String updateStock = "UPDATE san_pham SET so_luong_ton = ?, ngay_cap_nhat = GETDATE() WHERE ma_san_pham = ?";
            String insertHistory = "INSERT INTO lich_su_kho (ma_lich_su, ma_san_pham, loai_bien_dong, loai_tham_chieu, ma_tham_chieu, so_luong, ton_cu, ton_moi, ma_nguoi_tao, ngay_tao, ghi_chu) VALUES (?, ?, 'XUAT', 'PHIEU_XUAT', ?, ?, ?, ?, ?, GETDATE(), ?)";

            for (ReceiptItem item : items) {
                int oldStock = getCurrentStock(con, item.getProductId());
                if (item.getQuantity() > oldStock) {
                    throw new Exception("San pham ID " + item.getProductId() + " khong du ton kho de xuat.");
                }
                ...
                try (PreparedStatement ps = con.prepareStatement(insertHistory)) {
                    ...
                    ps.setLong(7, userId);
                    ps.setString(8, note != null && !note.trim().isEmpty() ? note : "Xuat kho tu phieu " + receiptCode);
                    ps.executeUpdate();
                }
            }
